# Filter the dSV table to remove SVs with low call rate and low MAF and remove samples with low call rate.
...
conv <- read.delim("/data/sv_name_conversion_table.txt", header = T,  sep = "\t", as.is = T, check.names = F)
