...
for c_num, c in enumerate(cohorts):
    with gzip.open("/data/umcg-tifn/SV/SV_GWAS/genotypes/" + c + "/" + c + "_filtered.frq.counts.gz", mode="rt") as frq_f:
        frq_f.readline()
        cnt = 0
        for l in frq_f:
            spl = l.strip().split()
...
out_f = open("/data/umcg-tifn/SV/SV_GWAS/results_all_summary_stats/", svtype, "/meta_combined/", species, "/", species, ".", svtype,".meta-analysis.annot.txt", "w")
...
with gzip.open("/data/umcg-tifn/SV/SV_GWAS/results_all_summary_stats/", svtype, "/meta_combined/", species, "/", species, ".", svtype,".meta-analysis.txt.gz", mode="rt") as f:
	#cnt = 0
	f.readline()
	for l in f:
		spl = l.rstrip().split()
