...
#corr.matrix<-abs(read.table("LLD_cov_matrix.tsv"))      # For multiple test correction the sign of the correlation is irrelevant (i.e., so we're best to input absolute values)
corr.matrix<-abs(read.table("LLD_cov_matrix_metabolites.tsv"))
