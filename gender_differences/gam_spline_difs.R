...
traits <- traits[,seq(3,ncol(traits))]
# expression
traits0 <- as.data.frame(t(read.table(gzfile("C:/Users/Dasha/work/UMCG/data/LifeLines_phenotypes/LLD_genelevel_readcounts.TMM.Log2Transformed.ProbesCentered.SamplesZTransformed.txt.gz"), header = T, row.names = 1, sep = "\t", as.is = T, check.names = F)))
gte <- read.table("C:/Users/Dasha/work/UMCG/data/LifeLines_phenotypes/gte_all.txt", sep = "\t", as.is = T, check.names = F)
...
gene_table <- read.table("C:/Users/Dasha/Downloads/geneid_to_gene.txt", header = T, sep = "\t", as.is = T, check.names = F)
