...
#d <- na.omit(scale(d))
cormat <- cor(t(d), method = "pearson", use="pairwise.complete.obs")
...
write.table(clusters, file = paste0("results/tables/nmr_clustering_hclust_orig_", num_k, ".8.v2022.txt"),sep = "\t", quote = F, row.names = F)
...
for (i in 1:max(cl)){
  ...
  draw_multiple_fitted_lines(as.data.frame(fitted_matrix[,lipids]), signif_inters, plot_title = paste0("cluster ", cnt, "\nN = ", length(lipids)))
  cnt <- cnt + 1
}
dev.off()
